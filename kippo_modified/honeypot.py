...
class HoneyPotShell(object):
    ...
    def lineReceived(self, line):
	#modified by koide
	com_wget = line.find("wget -c")
	if com_wget != -1:
		d= datetime.datetime.today()
		if line.find(";",com_wget) != -1:
			com_wget = line[com_wget:line.find(";",com_wget)]
		else:
			com_wget = line[com_wget:]
		filename = str(d.year)+str(d.month).zfill(2)+str(d.day).zfill(2)+str(d.hour).zfill(2)+str(d.minute).zfill(2)
		com_wget += " -O /opt/kippo/dl/wgetc/%s"%filename
		os.system("echo %s >> /opt/kippo/dl/wgetc/log"%(com_wget))
		os.system(com_wget)
		#subprocess.call(com_wget, shell=True )
